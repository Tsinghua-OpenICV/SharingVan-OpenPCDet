...
class Kitti_SceneflowDataset(Dataset):
    def __init__(self, npoints=16384, root='data/kitti_flow', train=True, cache=None):
        self.npoints = npoints
        self.train = train
        self.root = root
        if self.train:
...
# data
for epoch in range(0,150,10):
  net = Kitti_FlowNet3D().cuda()
  net.load_state_dict(torch.load('models/net_epoch_%d.pth' % epoch))
  net.eval()
#
# statistics
  ...
  with torch.no_grad():
    # for each mini-batch
      for points1, points2, features1, features2, flow, mask1 in test_loader:
        ...
        #
          pred_flow_sum = torch.zeros(2, 3, 16384).cuda(non_blocking=True)
        #
        #         # resample 10 times
          for i in range(10):
            #
...
#
  print('---')
  loss_rec.append(loss_sum / len(test_loader))
  epe_rec.append(epe_sum / len(test_loader))

print("loss_rec\n",loss_rec)
print("epe_rec\n",epe_rec)
f=open("loss_rec.txt","wb")
pickle.dump(loss_rec,f)
f.close()
f=open("epe_rec.txt","wb")
pickle.xump(epe_rec,f)
f.close()