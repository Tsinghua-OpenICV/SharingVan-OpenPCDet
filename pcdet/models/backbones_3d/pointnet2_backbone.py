...
class PointNet2MSG(nn.Module):
    ...
    def forward(self, batch_dict):
        ...
        points_auxi = batch_dict['points_auxi']
        batch_idx_auxi, xyz_auxi, features_auxi = self.break_up_pc(points_auxi)

        xyz_batch_cnt_auxi = xyz_auxi.new_zeros(batch_size).int()
        for bs_idx in range(batch_size):
            xyz_batch_cnt_auxi[bs_idx] = (batch_idx_auxi == bs_idx).sum()

        assert xyz_batch_cnt_auxi.min() == xyz_batch_cnt_auxi.max()
        xyz_auxi = xyz_auxi.view(batch_size, -1, 3)
        features_auxi = features_auxi.view(batch_size, -1, features_auxi.shape[-1]).permute(0, 2, 1) if features_auxi is not None else None

        l_xyz_auxi, l_features_auxi = [xyz_auxi], [features_auxi]
        for i in range(len(self.SA_modules)):
            li_xyz_auxi, li_features_auxi = self.SA_modules[i](l_xyz_auxi[i], l_features_auxi[i])
            l_xyz_auxi.append(li_xyz_auxi)
            l_features_auxi.append(li_features_auxi)

        for i in range(-1, -(len(self.FP_modules) + 1), -1):
            l_features_auxi[i - 1] = self.FP_modules[i](
                l_xyz_auxi[i - 1], l_xyz_auxi[i], l_features_auxi[i - 1], l_features_auxi[i]
            )  # (B, C, N)

        point_features_auxi = l_features_auxi[0].permute(0, 2, 1).contiguous()  # (B, N, C)
        batch_dict['point_features_auxi'] = point_features_auxi.view(-1, point_features_auxi.shape[-1])
        batch_dict['point_coords_auxi'] = torch.cat((batch_idx_auxi[:, None].float(), l_xyz_auxi[0].view(-1, 3)), dim=1)
        return batch_dict
